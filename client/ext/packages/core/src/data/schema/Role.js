...
Ext.define('Ext.data.schema.Role', {
    ...
    doGetFK: function(leftRecord, options, scope) {
        // Consider the Department entity with a managerId to a Region entity. This method
        // is the guts of the getManager method that we place on the Department entity to
        // acquire a Region entity. We are the "manager" role and that role describes a
        // Region. This method is called, however, given a Department (leftRecord) as the
        // start of this trek.
        ...
        if (!done) {
            // We don't have the Region record yet, so try to get it.
            if (session) {
                ...
                if (!foreignKeyId && foreignKeyId !== 0) {
                    ...
                    // make a new Region, set its idProperty and load the real record via
                    // Region.load method.
                    if (!rightRecord) {
                ...
                // Without a FK value by which to request the Region record, we cannot do
    ...
    doSetFK: function(leftRecord, rightRecord, options, scope) {
        ...
        // store the Region entity. We are the "manager" role and that role describes a
        // Region. This method is called, however, given a Department (record) and the Region
        // (value).
        var me = this,
            foreignKey = me.association.getFieldName(), // "managerId"
            instanceName = me.getInstanceName(), // "manager"
            ...
            inverse = me.inverse,
            inverseSetter = inverse.setterName, // setManagerDepartment for Region
