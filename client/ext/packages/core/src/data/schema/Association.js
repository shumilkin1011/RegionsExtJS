 ...
 * an ecommerce system `Region` can have many `Order` entities.
 *
 ...
 * (`B`). This is often used when partitioning data. For example a `Region` may have a single
 ...
 * of this can be seen  above for `Region/Order`.
 *
 ...
 * formats. A simple example definition using `Region/Order`:
 *
 *     Ext.define('Region', {
 *         extend: 'Ext.data.Model',
 *         fields: [{
 ...
 * + `Region` will have an `orders()` function that returns a `Ext.data.Store` of`Orders`.
 * + `Order` will have a `getUser` method which will return a `Region` Model.
 * + `Order` will have a `setUser` method that will accept a `Region` model or a key value.
 *
 * ## Nested Loading
 *
 * Nested loading is the ability to load hierarchical associated data from a remote source within
 * a single request. In the following example, each `Region` in the `users` store has an `orders`
 * store. Each `orders` store is populated with `Order` models read from the request. Each `Order`
 * model also has a reference back to the appropriate `Region`.
 *
