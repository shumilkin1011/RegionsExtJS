 ...
 * of Region and Group.
 * 
 *   * `entityName` - The names "Region" and "Group" are the `entityName` values associated
 *   with these two classes. These are derived from their full classnames (perhaps
 *   something like "App.model.Region" and "App.model.Group").
 *   
 ...
 *   "Region" and "Group", the default `associationName` would be "GroupUsers".
 *   
 ...
 *   as "Region" or "Group"). When discussing associations in the abstract, however, it is
 *   very helpful to be able to talk about the entities in an association in a general way.
 *   In the case of the "GroupUsers" association, "Region" is said to be the `left` while
...
Ext.define('Ext.data.schema.Schema', {
     ...
     * "users" for a `left.type` of "Region".
     * 
    ...
    privates: {
        ...
        addMatrixDescr: function(entityType, matrixName, matrixDef) {
            ...
            if (Ext.isString(matrixDef)) {
                if (matrixDef.charAt(0) === '#') { // "#Region" (entity is on the left)
                    /*
                     *  Ext.define('Region', {
                     *      extend: 'Ext.data.Model',
                     *      manyToMany: '#Group'
                     *  });
                     */
                    left = { type: entityName }; // Region
                    right = { type: matrixDef.substring(1) }; // Group
                }
                else if (matrixDef.charAt(last = matrixDef.length - 1) === '#') { // "Region#"
                    /*
                     *  Ext.define('Group', {
                     *      extend: 'Ext.data.Model',
                     *      manyToMany: 'Region#'
                     *  });
                     */
                    left = { type: matrixDef.substring(0, last) }; // Region
                ...
                if (left || (!right && namer.apply('multiRole', entityName) < namer.apply('multiRole', matrixDef.type))) {
                    if (!left || left === true) {
                         ...
                         *  });
                         */
                        left = Ext.apply({ type: entityName }, left); // Region
                    }
        ...
        applyDecoration: function(role) {
            var me = this,
                // To decorate a role like "users" (of a Region / Group matrix) we need to add
                // getter/setter methods to access the "users" collection ... to Group! All
                // other data about the "users" role and the Region class belong to the given
